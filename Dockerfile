...
RUN dotnet restore "MacronizerApi/MacronizerApi.csproj" -a $TARGETARCH -s https://api.nuget.org/v3/index.json --verbosity n

COPY . .
# Compile specifically for the target OS and Architecture
RUN dotnet build "MacronizerApi/MacronizerApi.csproj" -c Release -a $TARGETARCH -o /app/build

# Stage 3: publish
FROM build AS publish
ARG TARGETARCH
RUN dotnet publish "MacronizerApi/MacronizerApi.csproj" -c Release -a $TARGETARCH -o /app/publish
